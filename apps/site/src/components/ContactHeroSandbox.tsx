...
export function ContactHeroSandbox({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative">
      {/* Live apply via inline style on the image */}
      <style>{`
        .contact-hero-img {
      ...
      {/* Toggle + controls */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full z-30 pt-3">
        <button type="button" onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-1.5 rounded-full bg-amber-500 text-black px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider shadow-lg hover:bg-amber-400 transition-colors mx-auto">
          <Settings2 className="w-3 h-3" />
          {open ? "Close" : "Adjust Image"}
        </button>

        {open && (
          <div className="mt-2 bg-[#111] border border-amber-500/30 rounded-xl p-3 shadow-2xl w-[280px] space-y-3">
            {/* Device label */}
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                ...
                <button type="button" onClick={exportAll} className="text-[9px] text-amber-500 hover:text-amber-400 transition-colors">Copy All</button>
              </div>
            </div>
...
function Row({ label, value, children }: { label: string; value: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-[9px] font-semibold uppercase tracking-wider text-neutral-500 w-16 shrink-0">{label}</span>
      <span className="text-[10px] font-mono text-white tabular-nums w-14 text-center">{value}</span>
...
function Btn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick}
      className="grid h-5 w-5 place-items-center rounded border border-white/10 bg-white/5 text-neutral-400 hover:text-white hover:bg-white/10 transition-colors">
      {children}
    </button>
  );
}
